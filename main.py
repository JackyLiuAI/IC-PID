...
class PIDController:
    ...
    def compute(self, current_temp):
        ...
        self.D = self.Kd * delta_error  # 微分项（差分）
...
def simulate_boiler_temperature(pid, initial_temp, ambient_temp, heat_loss_coeff,
    ...
    for step in range(steps):
        ...
        print(f"步骤: {step+1}, 当前温度: {temperature:.2f}℃, 控制信号: {output:.2f}, "
              f"P: {P:.2f}, I: {I:.2f}, D: {D:.2f}, 控制时间: {dt*1000:.0f}ms")

    # 图1：温度 vs 控制步骤
    # plt.figure(figsize=(10, 6))
    # plt.plot(time_steps, temperatures, label='温度 (℃)')
    # plt.axhline(y=pid.setpoint, color='r', linestyle='--', label='目标温度')
    # plt.xlabel('控制步骤')
    # plt.ylabel('温度 (℃)')
    # plt.title('含散热和加水模型的锅炉温度随控制步骤变化')
    # plt.legend()
    # plt.grid(True)
    # plt.show()
    ...
    # 图2：温度 vs 累积时间
    plt.figure(figsize=(10, 6))
    plt.plot(real_times, temperatures, label='温度 (℃)')
    plt.axhline(y=pid.setpoint, color='r', linestyle='--', label='目标温度')
    plt.xlabel('累积时间 (秒)')
    plt.ylabel('温度 (℃)')
    plt.title('含散热和加水模型的锅炉温度随累积时间变化')
    plt.legend()
    plt.grid(True)
    plt.show()
...
simulate_boiler_temperature(pid, initial_temp=30, ambient_temp=ambient_temp,
                            heat_loss_coeff=heat_loss_coeff,
                            steps=300, cooling_n=cooling_n)
